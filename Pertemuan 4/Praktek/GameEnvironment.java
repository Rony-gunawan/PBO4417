...
public class GameEnvironment {
      ...
      public void getAllPlayers(){
            System.out.println("Playernya sapa aja : " + arrPlayer);
      }
      ...
      public void getAllEnemies(){
            System.out.println("Musuhnya sapa aja : " + arrEnemy);
      }
      public void Interaction (){
            if (arrPlayer == null || arrEnemy == null ){
                  System.out.println("Player atau musuhnya ada yang belum siap");
            }
            for (int i=0; i<arrPlayer.size();i++){
                 for(int j=0; j<arrEnemy.size();j++){
                       if(arrPlayer.get(i).getY() != arrEnemy.get(j).getY()){
                             System.out.println("Player : "+arrPlayer.get(i)+" dan Musuh: "+arrEnemy.get(j)+" Ga di posisi Y yang sama gan ");
                       }
                       if(EuclideanDistance(arrPlayer.get(i).getX(), arrPlayer.get(i).getY(),arrEnemy.get(j).getX(), arrEnemy.get(j).getY()) < 2){
                             ...
                             System.out.println("Musuhnya : "+arrEnemy.get(j)+" kalah");
